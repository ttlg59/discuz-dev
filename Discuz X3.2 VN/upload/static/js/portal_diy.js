...
drag.extend({
	...
	setDefalutMenu : function () {
			this.addMenu('default','Tiêu đề','drag.openTitleEdit(event)');
			this.addMenu('default','Phong cách','drag.openStyleEdit(event)');
			this.addMenu('default', 'Xóa', 'drag.removeBlock(event)');
			this.addMenu('block', 'Thuộc tính', 'drag.openBlockEdit(event)');
			this.addMenu('block', 'Dữ liệu', 'drag.openBlockEdit(event,"data")');
			this.addMenu('block', 'Cập nhật', 'drag.blockForceUpdate(event)');
			this.addMenu('frame', 'Xuất khung', 'drag.frameExport(event)');
			this.addMenu('tab', 'Xuất dữ liệu', 'drag.frameExport(event)');
	},
	...
	openStyleEdit : function (e) {
		...
		var stylearr = [{'key':'Kiểu viền','value':'none'},{'key':'Nét liền','value':'solid'},{'key':'Nét đứt chấm','value':'dotted'},{'key':'Nét đứt','value':'dashed'}];
		...
		table += '<tr><th>Đường viền</th><td><ul id="borderul" class="'+ulclass+'">';
		table += '<li><label>Trên</label><select class="ps vm" id="bdtwidth" ><option value="">Kích thước</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
		table += ' <select class="ps vm" id="bdtstyle" ><option value="">Phong cách</option>'+this.getOption(stylearr,bdtstyle)+'</select>';
		table += ' </br>Mã màu <input type="text" id="bdtcolor" class="px p_fre vm" value="'+bdtcolor+'" size="7" />';
		...
		table += ' <select class="ps vm" id="bdrstyle" ><option value="">Phong cách</option>'+this.getOption(stylearr,bdrstyle)+'</select>';
		table += ' </br>Mã màu <input type="text" id="bdrcolor" class="px p_fre vm" value="'+bdrcolor+'" size="7" />';
		...
		table += ' <select class="ps vm" id="bdbstyle" ><option value="">Phong cách</option>'+this.getOption(stylearr,bdbstyle)+'</select>';
		table += ' </br>Mã màu <input type="text" id="bdbcolor" class="px p_fre vm" value="'+bdbcolor+'" size="7" />';
		...
		table += ' <select class="ps vm" id="bdlstyle" ><option value="">Phong cách</option>'+this.getOption(stylearr,bdlstyle)+'</select>';
		table += ' </br>Mã màu <input type="text" id="bdlcolor" class="px p_fre vm" value="'+bdlcolor+'" size="7" />';
		table += getColorPalette(id+'_bdlPalette', 'bdlcolor' ,bdlcolor)+'</li>';
		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'">Thiết lập thêm</label></p></td></tr>';
		...
		table += '<tr><th>Canh lề trong</th><td><div id="margindiv" class="'+ulclass+'"><span><label>Trên</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
		table += '</br><span class="bordera"><label>Phải</label> <input type="text" id="marginr" class="px p_fre vm" value="'+marginr+'" size="1" />px </span>';
		table += '</br><span class="bordera"><label>Dưới</label> <input type="text" id="marginb" class="px p_fre vm" value="'+marginb+'" size="1" />px </span>';
		table += '</br><span class="bordera"><label>Trái</label> <input type="text" id="marginl" class="px p_fre vm" value="'+marginl+'" size="1" />px</span>';
		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">Thiết lập thêm</label></p></td></tr>';
		...
		if (objType == 1) {
			...
			table += '<tr><th>Canh lề ngoài</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>Trên</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Phải</label> <input class="px p_fre" id="cmarginr" value="'+cmarginr+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Dưới</label> <input class="px p_fre" id="cmarginb" value="'+cmarginb+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Trái</label> <input class="px p_fre" id="cmarginl" value="'+cmarginl+'" size="1" />px </span>';
			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">Thiết lập thêm</label></p></td></tr>';
		}
		table += '<tr><th>Màu nền</th><td><input type="text" id="bgcolor" class="px p_fre vm" value="'+bgcolor+'" size="4" />';
		table += getColorPalette(id+'_bgcPalette', 'bgcolor' ,bgcolor)+'</td></tr>';
		table += '<tr><th>Ảnh nền</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		var classarr = objType == 1 ? this.blockDefaultClass : this.frameDefaultClass;
		table += '<tr><th>Nền tiều đề</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';

		var wname = objType ? 'dữ liệu' : 'khung';
		html = '<div class="c diywin" style="width:450px;position:relative;">'+table+'</div>';
		var h = '<h3 class="flb"><em>Chỉnh sửa '+wname+'</em><span><a href="javascript:;" class="flbc" onclick="drag.closeStyleEdit(\''+id+'\');return false;" title="Đóng">\n\
			Close</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveStyle(\''+id+'\');drag.closeStyleEdit(\''+id+'\');" class="pn pnc" value="true">\n\
			<strong>Xác nhận</strong></button><button onclick="drag.closeStyleEdit(\''+id+'\')" class="pn" value="true"><strong>Hủy bỏ</strong></button></p>';
		this.removeMenu(e);
	...
	openTitleEdit : function (e) {
		...
		var titlename = obj instanceof Block ? 'dữ liệu' : 'khung';
		...
		common += '<tr><th>Màu ảnh nền:</th><td><input type="text" id="titleBgImage" class="px p_fre" value="'+bgimage+'" /> <select class="ps vm" id="titleBgRepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		common += '<tr><th>Màu nền:</th><td><input type="text" id="titleBgColor" class="px p_fre" value="'+bgcolor+'" size="7" />';
		common += getColorPalette(fid+'bgPalette_0', 'titleBgColor' ,bgcolor)+'</td></tr>';
		if (obj instanceof Tab) {
			var switchArr = [{'key':'Nhấn chuột','value':'click'},{'key':'Di chuột','value':'mouseover'}];
			var switchType = obj.titles['switchType'] ? obj.titles['switchType'][0] : 'click';
			common += '<tr><th>Loại:</th><td><select class="px p_fre vm" id="switchType" >'+this.getOption(switchArr,switchType)+'</select></td></tr>';
		}
		common += '</table><hr class="l">';
		var li = '';
		li += '<div id="titleInput_0"><table class="tfm"><tr><th>Tiêu đề '+titlename+':</th><td><input type="text" id="titleText_0" class="px p_fre" value="`title`" /></td></tr>';
		li += '<tr><th>Liên kết:</th><td><input type="text" id="titleLink_0" class="px p_fre" value="`link`" /></td></tr>';
		li += '<tr><th>Link ảnh:</th><td><input type="text" id="titleSrc_0" class="px p_fre" value="`src`" /></td></tr>';
		li += '<tr><th>Vị trí:</th><td><select id="titleFloat_0" class="ps vm"><option value="" `left`>Căn trái</option><option value="right" `right`>Căn phải</option></select>';
		li += '&nbsp;&nbsp;Khoảng cách: <input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="7" />px</td></tr>';
		li += '<tr><th>Kiểu chữ:</th><td><select class="ps vm" id="titleSize_0" ><option value="">Cỡ chữ</option>`size`</select>';
		li += '&nbsp;&nbsp;Màu chữ: <input type="text" id="titleColor_0" class="px p_fre vm" value="`color`" size="4" />';
		...
		var c = len + 1;
			html = '<div class="c diywin" style="width:450px;height:400px; overflow:auto;"><table cellspacing="0" cellpadding="0" class="tfm pns"><tr><th></th><td><button type="button" id="addTitleInput" class="pn" onclick="drag.addTitleInput('+c+');"><em>Thêm tiêu đề mới</em></button></td></tr></table><div id="titleEdit">'+html+common+'</div></div>';
		var h = '<h3 class="flb"><em>Chỉnh sửa tiêu đề '+titlename+'</em><span><a href="javascript:;" class="flbc" onclick="drag.closeTitleEdit(\''+fid+'\');return false;" title="Close">\n\
			Đóng</a></span></h3>';
		var f = '<p class="o pns"><button onclick="drag.saveTitleEdit(\''+fid+'\');drag.closeTitleEdit(\''+fid+'\');" class="pn pnc" value="true">\n\
			<strong>Xác nhận</strong></button><button onclick="drag.closeTitleEdit(\''+fid+'\')" class="pn" value="true"><strong>Cancel</strong></button></p>';
		this.removeMenu(e);
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('Bạn muốn xóa nó sao? Sau khi xóa sẽ không thể phục hồi lại dữ liệu.')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>Đang cập nhệt dữ liệu....</center>';
		var x = new Ajax();
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = 'Total <span style="color:blue">'+this.blocksLen+'</span> Dữ liệu đã được cập nhật<span style="color:red">'+cur+'</span>Dữ liệu đã cập nhật xong.<span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
	...
	getTabHtml : function () {
		...
		var str = '<div id="'+id+'" class="'+className+'">';
		str += '<div id="'+id+'_title" class="'+titleClassName+'"><span class="'+this.titleTextClass+'">Thẻ Tab</span></div>';
