...
function loadcalendar() {
	...
	s += '<th><a href="###" onclick="refreshcalendar(yy, mm-1)" title="Ngày tiếp theo">《</a></th>';
	s += '<th colspan="5" style="text-align: center"><a href="###" onclick="showdiv(\'year\');_cancelBubble(event)" title="Nhấp vào  để chọn Năm" id="year"></a>&nbsp; - &nbsp;<a id="month" title="Nhấp vào để chọn tháng" href="###" onclick="showdiv(\'month\');_cancelBubble(event)"></a></th>';
	s += '<th><A href="###" onclick="refreshcalendar(yy, mm+1)" title="Tháng tiếp theo">》</A></th>';
	s += '</tr>';
	s += '<tr id="calendar_header"><td>Ngày</td><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td><td>6</td></tr>';
	s += '</thead>';
	s += '<tbody>';
	for(var i = 0; i < 6; i++) {
		s += '<tr>';
	...
	s += '<tr id="hourminute"><td colspan="7" align="center"><input type="text" size="2" value="" id="hour" onKeyUp=\'this.value=this.value > 23 ? 23 : zerofill(this.value);controlid.value=controlid.value.replace(/\\d+(\:\\d+)/ig, this.value+"$1")\'> 点 <input type="text" size="2" value="" id="minute" onKeyUp=\'this.value=this.value > 59 ? 59 : zerofill(this.value);controlid.value=controlid.value.replace(/(\\d+\:)\\d+/ig, "$1"+this.value)\'> Điểm</td></tr>';
	s += '</tbody>';
	s += '</table></div></div>';
