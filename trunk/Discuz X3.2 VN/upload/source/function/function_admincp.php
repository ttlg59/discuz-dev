...
function showsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '', $setid = '', $nofaq = false) {
	...
	if(!isset($_G['showsetting_multi'])) {
		if(!$nofaq) {
			$faqurl = 'http://help.discuzvn.com?type=admin&ver='.$_G['setting']['version'].'&action='.rawurlencode($_GET['action']).'&operation='.rawurlencode($_GET['operation']).'&key='.rawurlencode($setname);
...
<script type="text/javascript">
	var newhtml = '';
	newhtml += '<table class="tb tb2"><tr><th class="partition edited">Kiểm tra phiên bản discuz.</th></tr>';
