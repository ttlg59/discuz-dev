...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		if(is_err_t) {
			var parentnode = obj.parent();
			parentnode.find('.loading').remove();
			parentnode.append('<div class="error_text">Tải lại ảnh</div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(typeof pop == 'string') {
			$('#ntcmsg').remove();
			if(type == 'alert') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="button2" type="button" value="Đóng" onclick="popup.close();"></dd></div>'
			} else if(type == 'confirm') {
				pop = '<div class="tip"><dt>'+ pop +'</dt><dd><input class="redirect button2" type="button" value="Đóng" href="'+ url +'"><a href="javascript:;" onclick="popup.close();">取消</a></dd></div>'
			}
...
var geo = {
	...
	locationerror : function(error) {
		...
		switch(error.code) {
			case error.TIMEOUT:
				geo.errmsg = "Quá thời gian định vị địa điểm, hãy thử lại";
				break;
			case error.POSITION_UNAVAILABLE:
				geo.errmsg = 'Không thể xác định vị trí của bạn';
			    break;
		    case error.PERMISSION_DENIED:
		        geo.errmsg = 'Hãy đồng ý cho phép truy cập ví trí của bạn';
		        break;
		    case error.UNKNOWN_ERROR:
		        geo.errmsg = 'Lỗi không rõ ràng';
		        break;
		}
	},
	...
	init : function() {
		...
		.on('touchmove', function(e) {
			...
			if(!status && $(window).scrollTop() <= 0) {
				...
				contentobj.html('<img src="'+ STATICURL + 'image/mobile/images/icon_arrow.gif" style="vertical-align:middle;margin-right:5px;-moz-transform:rotate(180deg);-webkit-transform:rotate(180deg);-o-transform:rotate(180deg);transform:rotate(180deg);"><span id="refreshtxt">Kéo xuống để tải lại trang</span>');
			...
			if(status == true) {
				...
				if(pullheight >= 0 && pullheight < 150) {
					...
					if(reloadflag) {
						...
						contentobj.find('#refreshtxt').html('Kéo xuống để tải lại trang');
					}
					reloadflag = false;
					...
					if(!reloadflag) {
						...
						contentobj.find('#refreshtxt').html('Nhả để làm mới');
					}
					reloadflag = true;
				}
			}
